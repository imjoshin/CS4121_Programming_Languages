...
;added
(define-syntax-rule (define-monster id
                      desc
                      (path ...)
                      ([vrb expr] ...))
  (begin
...
(define places null)          ; list of verbs
(define monsters null)          ; list of verbs
...
;; Try to drop a passed item
(define (try-drop! t)
  ; if trying to drop backpack and there are items, dont allow it
  (if (and  (equal? (thing-name t) 'backpack) (> (length stuff) 2))
      (printf "You cannot drop the backpack with items inside.\n")
      ; if trying to drop a valid item
      (begin
        ; drop if user has it
        (if (have-thing? t)
            (begin
              (drop-thing! t)
              (printf "You have dropped the ~a.\n" (thing-name t))
            )
            (printf "You don't have the ~a.\n" (thing-name t))
        )
      )
  )
)

;; Try to take a passed item
(define (try-take! t)

  ; if they already have it, don't allow taking
  (if (have-thing? t)
      (printf "You already have the ~a.\n" (thing-name t))
      ; if they don't have it
      (begin
        ; check if they are already holding an item without a backpack
        (if (and (> (length stuff) 0) (not (equal? (thing-name t) 'backpack)) (not (have-thing-name? 'backpack)))
            (printf "You cannot hold more than one item without a backpack.\n")
            (begin 
              (take-thing! t)
              (printf "You now have the ~a.\n" (thing-name t))
              )
            )
        )
      )
...
;; Look for item in inventory with a given name
(define (have-thing-name? t)
  (define ret #f)
  ; loop through stuff and check thing-name
  (for-each (lambda (thing)
              (if (equal? (thing-name thing) t) (set! ret #t) (printf "")))
            stuff)
...
(define (show-current-place)
  ...
  (for-each (lambda (thing)
              (if (not (equal? (thing-name thing) 'door)) (printf "There is a ~a here.\n" (thing-name thing)) (printf "")))
...
;added
(define (teleport)
  (printf "You encountered the Demented Professor!\n")
  (display "Number of places: ")
  (displayln (length places))
  ;(define places '())
  ;(set! current-place (get-random places))
  ;(do-place)
