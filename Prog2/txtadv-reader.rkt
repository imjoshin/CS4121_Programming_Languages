...
(define (txtadv-read-syntax src in)
  ...
  (expect-section src in "MONSTERS")
  (define monsters (in-section src in read-monster))
  (datum->syntax
   #f
...
;; Check for valid place name
(define (read-place-name src in)
  (if (regexp-match-peek #px"^[A-Za-z0-9-]+(?=:$|\\s|[],])" in)
      (read-syntax src in)
      (complain src in "expected a place")))

;; Read monster path
(define (read-place-sequence src in)
  (let loop ([names null])
    (define s (read-place-name src in))
    (if (regexp-match-peek #rx"^, " in)
        (begin
          (read-char in)
          (read-char in)
          (loop (cons s names)))
        (values (reverse (cons s names)) #f))))
...
;; Read in each monster
(define (read-monster src in)
  ; Monster name
  (define name (read-defn-name src in "monster"))
  (skip-whitespace in)
  ; Monster description
  (define desc (if (regexp-match-peek #rx"^\"" in)
                   (read-syntax src in)
  ...
  ;; Start looking for path
    (unless (regexp-match-peek #rx"^[[]" in)
    (complain src in "expected a square bracket to start a list of places for a monster"))
  (read-char in)5
  (define-values (path _) 
    (if (regexp-match-peek #rx"^[]]" in)
        (values null #f)
        (read-place-sequence src in)))
  (unless (regexp-match-peek #rx"^[]]" in)
    (complain src in "expected a square bracket to end a list of places for a monster"))
  (read-char in)
  ; Create monster structure
  (define actions (in-defn src in read-action))
  `(define-monster ,name ,desc ,path ,actions))